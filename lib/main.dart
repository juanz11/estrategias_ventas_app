...
import 'mixins/data_loader_mixin.dart';
import 'mixins/ingresos_mixin.dart';
import 'mixins/gastos_mixin.dart';
import 'mixins/deudas_mixin.dart';
import 'mixins/categorias_mixin.dart';
...
class _GastoDraft {
  ...
  GastoPeriodicidad periodicidad;
  bool esFijo;
  bool pagoConTarjeta;
  bool gastoHormiga;
  ...
  void _addDraft() {
    _drafts.add(
      _GastoDraft(
        ...
        periodicidad: GastoPeriodicidad.mensual,
        esFijo: false,
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 12),
                          DropdownButtonFormField<GastoPeriodicidad>(
                            value: d.periodicidad,
                            decoration: const InputDecoration(
                              labelText: 'Periodicidad',
                              prefixIcon: Icon(Icons.calendar_month_outlined),
                            ),
                            items: GastoPeriodicidad.values
                                .map(
                                  (v) => DropdownMenuItem(
                                    value: v,
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      // Navegar primero, luego actualizar estado
...
class _GastosGroupedCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final grouped = <String, List<MapEntry<int, GastoMensual>>>{};
                      ...
                      Row(
                        children: [
                          ...
                          PopupMenuButton<String>(
                            ...
                            onSelected: (value) {
                              if (value == 'agregar') onAddToCategoria(cat);
                              if (value == 'editar') onEditCategoria?.call(cat);
                            },
                            itemBuilder: (_) => [
                              const PopupMenuItem(value: 'agregar', child: ListTile(leading: Icon(Icons.add), title: Text('Agregar gasto'))),
                              if (onEditCategoria != null)
                                const PopupMenuItem(value: 'editar', child: ListTile(leading: Icon(Icons.edit_outlined), title: Text('Renombrar'))),
                            ],
                          ),
                        ],
                      ),
...
class _DashboardScreenState extends State<DashboardScreen>
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        initializeData().then((_) {
          if (mounted) _verificarDeudasHoy();
        });
      }
    });
  }
  ...
  @override
  List<IngresoMensual> ingresosPresupuesto = [];

  @override
  List<GastoMensual> gastosPresupuesto = [];

  // Datos REALES de OPERACIONES (ingresos y gastos reales por mes)
  @override
  List<IngresoMensual> ingresosReales = [];

  @override
  List<GastoMensual> gastosReales = [];

  @override
  Set<String> categoriasGasto = {};

  @override
  List<Deuda> deudas = [];

  // Totales de ingresos y gastos REALES (de Operaciones - TODOS los meses)
  double get _totalIngresosReales => ingresosReales.fold(0, (sum, it) => sum + it.monto);

  double get _totalGastosReales => gastosReales.fold(0, (sum, it) => sum + it.monto);
  ...
  double get _totalPresupuestoIngresos => ingresosPresupuesto.fold(0, (sum, it) => sum + it.monto);

  double get _totalPresupuestoGastos => gastosPresupuesto.fold(0, (sum, it) => sum + it.monto);

  double get _totalDeudasPorPagar => deudas
      .where((d) => d.tipo == TipoDeuda.porPagar)
      .fold(0.0, (sum, d) => sum + d.monto);

  double get _totalDeudasPorCobrar => deudas
      .where((d) => d.tipo == TipoDeuda.porCobrar)
      .fold(0.0, (sum, d) => sum + d.monto);
  ...
  Future<void> _addDeuda() async {
    ...
    final deuda = result['deuda'] as Deuda;
    final archivo = result['archivo'] as File?;
    
    await addDeuda(deuda, archivo: archivo);
    
    final hoy = DateTime.now();
    final esHoy = deuda.fecha.year == hoy.year &&
        deuda.fecha.month == hoy.month &&
        deuda.fecha.day == hoy.day;
    
    if (esHoy) _mostrarAlertaDeuda(deuda);
  }

  Future<void> _editDeuda(int index) async {
    final result = await _openDeudaModal(context, initial: deudas[index]);
    if (!mounted || result == null) return;
        ...
        content: Column(
          ...
          children: [
            ...
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                color: deuda.tipo == TipoDeuda.porPagar
  ...
  void _verificarDeudasHoy() {
    final hoy = DateTime.now();
    final deudasHoy = deudas.where((deuda) {
  ...
  void _mostrarAlertasDeudasHoy(List<Deuda> deudasHoy) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Row(
  ...
  Future<void> _addIngresoPresupuesto() async {
    ...
    if (!mounted || result == null) return;
    await addIngresoPresupuesto(result);
  }

  Future<void> _editIngresoPresupuesto(int index) async {
    final result = await _openIngresoModal(context, initial: ingresosPresupuesto[index]);
    if (!mounted || result == null) return;
    await editIngresoPresupuesto(index, result);
  }

  void _deleteIngresoPresupuesto(int index) {
    deleteIngresoPresupuesto(index);
  }

  Future<void> _editGastoPresupuesto(int index) async {
    final result = await _openGastoModal(context, initial: gastosPresupuesto[index]);
    if (!mounted || result == null) return;
    final gasto = result['gasto'] as GastoMensual;
    final archivo = result['archivo'] as File?;
    await editGastoPresupuesto(index, gasto, archivo: archivo);
  }
  ...
  Future<void> _deleteGastoPresupuesto(int index) async {
    ...
    if (ok != true) return;
    if (!mounted) return;
    await deleteGastoPresupuesto(index);
  }
  ...
  Future<void> _addIngresoReal() async {
    final result = await _openIngresoModal(context);
    if (!mounted || result == null) return;
    await addIngresoReal(result);
  }

  Future<void> _editIngresoReal(int index) async {
    final result = await _openIngresoModal(context, initial: ingresosReales[index]);
    if (!mounted || result == null) return;
  ...
  Future<void> _addGastoReal() async {
    ...
    final gasto = result['gasto'] as GastoMensual;
    final archivo = result['archivo'] as File?;
    await addGastoReal(gasto, archivo: archivo);
  }

  Future<void> _editGastoReal(int index) async {
    final result = await _openGastoModal(context, initial: gastosReales[index]);
    if (!mounted || result == null) return;
    final gasto = result['gasto'] as GastoMensual;
    final archivo = result['archivo'] as File?;
  ...
  Future<void> _addGastosToCategoria(String categoria) async {
    ...
    for (final gasto in result) {
      await addGastoPresupuesto(gasto);
    }
  }
  ...
  Future<void> _editCategoriaGasto(String categoriaActual) async {
    final nuevoNombre = await _openRenombreCategoriaModal(context, categoriaActual);
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => _IngresosMensualesScreen(
          ...
          onEditCategoria: _editCategoriaGasto,
        ),
      ),
    ).then((_) {
  ...
  Widget build(BuildContext context) {
    ...
    final views = [
      _ResumenTab(
        ...
        gastos: gastosReales,
        ingresosPresupuesto: ingresosPresupuesto,
        gastosPresupuesto: gastosPresupuesto,
        onOpenIngresos: _openIngresosScreen,
      ),
      _PresupuestoTab(
        ingresos: ingresosPresupuesto,
        gastos: gastosPresupuesto,
        ingresosReales: _totalIngresosReales,
        gastosReales: _totalGastosReales,
        categorias: categoriasGasto.toList(),
          ...
          child: TabBarView(
            ...
            children: [
              ...
              _GastosPresupuestoView(
                ...
                onEditCategoria: widget.onEditCategoria,
                ...
                children: [
                  ...
                  if (cats.isEmpty)
                    ...
                    ListView.separated(
                      ...
                      itemBuilder: (context, i) {
                        final cat = cats[i];
                        final entries = <MapEntry<int, GastoMensual>>[];
                              ...
                              Row(
                                children: [
                                  ...
                                  PopupMenuButton<String>(
                                    icon: const Icon(Icons.more_vert),
                                    onSelected: (value) {
                                      if (value == 'editar') onEditCategoria(cat);
                                      if (value == 'eliminar') onDeleteCategoria(cat);
                                      if (value == 'agregar') onAddToCategoria(cat);
                                    },
                                    itemBuilder: (_) => const [
                                      PopupMenuItem(value: 'agregar', child: ListTile(leading: Icon(Icons.add), title: Text('Agregar gasto'))),
                                      PopupMenuItem(value: 'editar', child: ListTile(leading: Icon(Icons.edit_outlined), title: Text('Renombrar'))),
                                      PopupMenuItem(value: 'eliminar', child: ListTile(leading: Icon(Icons.delete_outline), title: Text('Eliminar'))),
                                    ],
                                  ),
                                ],
                              ),
...
class _EstadisticasTab extends StatefulWidget {
  ...
  final List<IngresoMensual> ingresos;
  final List<GastoMensual> gastos;
  final List<IngresoMensual> ingresosPresupuesto;
  final List<GastoMensual> gastosPresupuesto;
...
class _ResumenTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalIngresosPresupuesto = ingresosPresupuesto.fold(0.0, (sum, i) => sum + i.monto);
    final totalGastosPresupuesto = gastosPresupuesto.fold(0.0, (sum, g) => sum + g.monto);
    final gananciasPresupuestadas = totalIngresosPresupuesto - totalGastosPresupuesto;
    
    // Calcular totales de OPERACIONES REALES
    final totalIngresosReales = ingresos.fold(0.0, (sum, i) => sum + i.monto);
    final totalGastosReales = gastos.fold(0.0, (sum, g) => sum + g.monto);
    final gananciasReales = totalIngresosReales - totalGastosReales;
    
    // Calcular KPIs del MES ACTUAL (solo datos reales del mes actual)
    final ahora = DateTime.now();
    final ingresosDelMes = ingresos.where((i) => 
      i.mes.year == ahora.year && i.mes.month == ahora.month
    ).fold(0.0, (sum, i) => sum + i.monto);
    
    final gastosDelMes = gastos.where((g) => 
      g.mes.year == ahora.year && g.mes.month == ahora.month
    ).fold(0.0, (sum, g) => sum + g.monto);
    
    final balanceDelMes = ingresosDelMes - gastosDelMes;
    final ahorroDelMes = ingresosDelMes > 0 
      ? (balanceDelMes / ingresosDelMes * 100) 
      : 0.0;
      ...
      child: Column(
        ...
        children: [
                        ...
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 4),
                              Text(
                                'Ingresos: ${_money(totalIngresosReales)}',
                                style: const TextStyle(
                                  fontSize: 10,
                                  color: Colors.black54,
                                ),
                              ...
                              Text(
                                'Gastos: ${_money(totalGastosReales)}',
                                style: const TextStyle(
                                  fontSize: 10,
                                  color: Colors.black54,
                                ),
          ...
          _CardSection(
            ...
            child: SizedBox(
              height: 220, 
              child: _DistribucionGastosChart(
                gastos: gastos,
              ),
            ),
          ),
...
enum IngresoTipo { fija, variable, sinEspecificar }

enum GastoPeriodicidad {
  diario,
  semanal,
  quincenal,
  mensual,
...
class GastoMensual {
  ...
  final GastoPeriodicidad periodicidad;
  final DateTime mes;
  ...
  factory GastoMensual.fromJson(Map<String, dynamic> json) {
    return GastoMensual(
      id: json['id'],
      categoria: json['categoria'],
      subCategoria: json['sub_categoria'],
      monto: double.parse(json['monto'].toString()),
      esFijo: json['es_fijo'] ?? false,
      pagoConTarjeta: json['pago_con_tarjeta'] ?? false,
      gastoHormiga: json['gasto_hormiga'] ?? false,
      periodicidad: GastoPeriodicidad.values.firstWhere(
        (e) => e.name == json['periodicidad'],
        orElse: () => GastoPeriodicidad.mensual,
      ),
      mes: DateTime.parse(json['mes']),
      archivoUrl: json['archivo_url'],
      archivoNombre: json['archivo_nombre'],
      archivoTipo: json['archivo_tipo'],
    );
  }
...
class IngresoMensual {
  ...
  final IngresoTipo tipo;
  final double monto;
  final DateTime mes;

  IngresoMensual copyWith({
    int? id,
    String? etiqueta,
    IngresoTipo? tipo,
    double? monto,
  ...
  factory IngresoMensual.fromJson(Map<String, dynamic> json) {
    return IngresoMensual(
      id: json['id'],
...
class Deuda {
  ...
  final String? archivoUrl;
  final String? archivoNombre;
  final String? archivoTipo; // 'image' o 'pdf'
  ...
  factory Deuda.fromJson(Map<String, dynamic> json) {
    return Deuda(
      id: json['id'],
      nombre: json['concepto'],
      monto: double.parse(json['monto'].toString()),
      tipo: json['tipo'] == 'por_pagar' ? TipoDeuda.porPagar : TipoDeuda.porCobrar,
  ...
  Map<String, dynamic> toJson() {
    return {
      'concepto': nombre,
      'monto': monto,
      'tipo': tipo == TipoDeuda.porPagar ? 'por_pagar' : 'por_cobrar',
...
class _IngresosMensualesScreenState extends State<_IngresosMensualesScreen> {
  ...
  Future<void> _editCategoriaAndRefresh(String categoria) async {
    await widget.onEditCategoria(categoria);
    if (!mounted) return;
    setState(() {});
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: _IngresosMensualesView(
          ...
          onEditCategoria: _editCategoriaAndRefresh,
        ),
      ),
    );
  }
                          ...
                          child: Row(
                            children: [
                              Container(
                                ...
                                child: Icon(
                                  it.tipo == IngresoTipo.fija
                                      ? Icons.lock_outline
                                      : it.tipo == IngresoTipo.variable
                                      ? Icons.swap_horiz
                                      : Icons.help_outline,
                                  color: colorScheme.primary,
                                ),
...
class _IngresoModalState extends State<_IngresoModal> {
  ...
  void initState() {
    ...
    _tipo = widget.initial?.tipo ?? IngresoTipo.sinEspecificar;
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              DropdownButtonFormField<IngresoTipo>(
                value: _tipo,
                decoration: const InputDecoration(
                ...
                items: const [
                  DropdownMenuItem(value: IngresoTipo.fija, child: Text('Fija')),
                  DropdownMenuItem(
                    value: IngresoTipo.variable,
                    child: Text('Variable'),
                  ),
                  DropdownMenuItem(
                    value: IngresoTipo.sinEspecificar,
...
class _GastoModalState extends State<_GastoModal> {
  ...
  late GastoPeriodicidad _periodicidad;
  late DateTime _fecha;
  ...
  void initState() {
    ...
    _subCategoria = TextEditingController(text: widget.initial?.subCategoria ?? '');
    _monto = TextEditingController(
      text: widget.initial == null ? '' : widget.initial!.monto.toStringAsFixed(2),
    );
    ...
    _periodicidad = widget.initial?.periodicidad ?? GastoPeriodicidad.mensual;
    _fecha = widget.initial?.mes ?? DateTime.now();
  }
  ...
  void _save() {
    ...
    Navigator.of(context).pop({'gasto': item, 'archivo': _archivoSeleccionado});
  }
          ...
          child: Column(
            ...
            children: [
              ...
              OutlinedButton.icon(
                ...
                label: Text(
                  _archivoNombre ??
                      (widget.initial?.archivoNombre != null
                          ? 'Cambiar: ${widget.initial!.archivoNombre}'
...
Future<String?> _openRenombreCategoriaModal(BuildContext context, String nombreActual) {
...
Future<List<GastoMensual>?> _openMultiGastoModal(
  BuildContext context, {
  required String categoria,
}) {
  return showModalBottomSheet<List<GastoMensual>>(
    context: context,
...
Future<Map<String, dynamic>?> _openGastoModal(
  BuildContext context, {
  GastoMensual? initial,
}) {
...
Future<IngresoMensual?> _openIngresoModal(
  BuildContext context, {
  IngresoMensual? initial,
}) {
  return showModalBottomSheet<IngresoMensual>(
    context: context,
    isScrollControlled: true,
    showDragHandle: true,
    builder: (ctx) {
...
String _tipoLabel(IngresoTipo tipo) {
  switch (tipo) {
    case IngresoTipo.fija:
      return 'Fija';
    case IngresoTipo.variable:
      return 'Variable';
    case IngresoTipo.sinEspecificar:
      return 'Sin especificar';
  }
}

String _periodicidadLabel(GastoPeriodicidad v) {
  switch (v) {
    case GastoPeriodicidad.diario:
      return 'Diario';
    case GastoPeriodicidad.semanal:
      return 'Semanal';
    case GastoPeriodicidad.quincenal:
      return 'Quincenal';
    case GastoPeriodicidad.mensual:
      return 'Mensual';
    case GastoPeriodicidad.bimestral:
      return 'Bimestral';
    case GastoPeriodicidad.trimestral:
      return 'Trimestral';
    case GastoPeriodicidad.semestral:
      return 'Semestral';
    case GastoPeriodicidad.anual:
      return 'Anual';
  }
}
  ...
  Widget build(BuildContext context) {
    final items = [
      ...
      _Kpi(
        ...
        color: balance >= 0 ? const Color(0xFF2E7D32) : const Color(0xFFD32F2F),
      ),
      _Kpi(
        title: 'Ahorro',
        value: '${ahorro.toStringAsFixed(1)}%',
        delta: 'De ingresos',
  ...
  showDialog(
    ...
    builder: (_) => Dialog(
      child: Column(
        ...
        children: [
          AppBar(
            title: Text(deuda.archivoNombre ?? 'Archivo adjunto'),
            ...
            Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text(
                    deuda.archivoNombre ?? 'Documento PDF',
                    textAlign: TextAlign.center,
                    style: const TextStyle(fontWeight: FontWeight.w700),
                  ),
                  const SizedBox(height: 16),
                  FilledButton.icon(
                    onPressed: () async {
                      final uri = Uri.parse(deuda.archivoUrl!);
...
Future<Map<String, dynamic>?> _openDeudaModal(BuildContext context, {Deuda? initial}) {
  return showModalBottomSheet<Map<String, dynamic>>(
    context: context,
    isScrollControlled: true,
...
class _DeudaModalState extends State<_DeudaModal> {
  ...
  void initState() {
    ...
    _tipo = widget.initial?.tipo ?? TipoDeuda.porPagar;
  ...
  void _save() {
    ...
    final deuda = Deuda(
      ...
      archivoUrl: widget.initial?.archivoUrl,
      archivoNombre: widget.initial?.archivoNombre,
      archivoTipo: widget.initial?.archivoTipo,
    );

    // Retornar deuda y archivo seleccionado juntos
    Navigator.of(context).pop({'deuda': deuda, 'archivo': _archivoSeleccionado});
  }
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            DropdownButtonFormField<TipoDeuda>(
              ...
              items: const [
                DropdownMenuItem(
                  value: TipoDeuda.porPagar,
                  child: Text('Por Pagar'),
                ),
                DropdownMenuItem(
                  value: TipoDeuda.porCobrar,
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: FilledButton(
                    onPressed: _subiendoArchivo ? null : _save,
                    child: const Text('Guardar'),
                  ),
                ),
              ],
...
class _DistribucionIngresosChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ingresosFijos = ingresos
        .where((i) => i.tipo == IngresoTipo.fija)
        .fold(0.0, (sum, i) => sum + i.monto);
